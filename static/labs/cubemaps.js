...
const ui={
  ...
  dx:$('dx'),dy:$('dy'),dz:$('dz'),dxText:$('dxText'),dyText:$('dyText'),dzText:$('dzText'),directionInfo:$('directionInfo'),sampleFace:$('sampleFace'),
...
function updateUI(){
  ...
  ui.dxText.textContent=(+ui.dx.value).toFixed(2);ui.dyText.textContent=(+ui.dy.value).toFixed(2);ui.dzText.textContent=(+ui.dz.value).toFixed(2);const x=+ui.dx.value,y=+ui.dy.value,zv=+ui.dz.value,h=hitFace(x,y,zv),len=Math.hypot(x,y,zv);ui.sampleFace.textContent=h.face;ui.directionInfo.innerHTML=`direction = (${x.toFixed(2)}, ${y.toFixed(2)}, ${zv.toFixed(2)})<br>length = ${len.toFixed(2)} <span style="color:#8b949e">(长度不决定 face)</span><br>dominant axis → <strong>${h.face}</strong> → ${h.file}`;document.querySelectorAll('.facepill').forEach(el=>el.classList.toggle('hit',el.dataset.face===h.face));ui.ior.disabled=ui.material.value!=='refract';buildWorkGrid();updateCode();
}
